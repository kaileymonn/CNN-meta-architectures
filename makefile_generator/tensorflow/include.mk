...
#   make        - builds recipe for `default`. This in turn prepares recipe
...
#   make data   - only procure data artifacts required for dag building
#   make tool   - only build CnnGen from source code procured from repo source
#                 ssh://$(USER)@ambus-git.ambarella.com:29418/cvtools/manifest
...
#                 usage: check tensorflow/inception/Makefile.in
...
#                 e.g. -c coeff-force-fx16 -idf 1,0,5,0
...
## encode output ambarella artifacts to be generated
NN_NAME     ?= inception
...
LABEL_CSV   ?= /cv1/sequence/data_set/imagenet/test-images/ILSVRC2012_validation_ground_truth_1001.csv
...
### using TF as the default parser.
NN_PARSER   ?= $(TF_TOOL)
# - setting up TF options as the default parser options
PARSER_OPTS ?= $(TF_OPTS)
...
data:
	...
	mkdir -p $(IP_DIR)
	@echo "-----(info) copying protobuf file"
...
ifeq ("$(wildcard $(IREG_PATH)/.)", "")
	...
	time ades_regression.py --vasbasename $(NN_NAME)	\
		--path $(OP_DIR)	\
...
ifeq ("$(USE_LOCAL_PARSER)", "1")
	$(eval TF_TOOL = "$(PARSERS_DIR)/tensorflow/tfparser.py")
	@chmod +x $(NN_PARSER)
endif
